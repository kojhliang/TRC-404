...
(async () => {
  ...
  // //**********5.invoke contract (test changeFreemintConfig and ChangeRoyaltyParams and ChangeOwnedNftLimit)*******************/
  let freemint_flag = -1;  // -1:true,0:false
  let freemint_max_supply =1500;
  let freemint_price =1 ; //1 tonn
  let changeFreemintConfig_TxSeqo = await invokeChangeFreemintConfigFromTrc404Master(user1_wallet, user1_secretKey,
    deployMasterContractAddress, freemint_flag, freemint_max_supply, freemint_price, "0.01"); 
  console.log("userAddress:", user1_wallet.address.toString(), ",userTxSeqo:", changeFreemintConfig_TxSeqo);
  await waitNextSeqo(user1_wallet, changeFreemintConfig_TxSeqo);
  ...
  //********ChangeOwnedNftLimit */
    let owned_nft_limit=6;
    let changeOwnedNftLimit_TxSeqo = await invokeChangeOwnedNftLimitFromTrc404Collection(user1_wallet, user1_secretKey,
    deployTrc404CollectionContract,owned_nft_limit,  "0.01"); 
    console.log("userAddress:", user1_wallet.address.toString(), ",userTxSeqo:", changeOwnedNftLimit_TxSeqo);
    await waitNextSeqo(user1_wallet, changeOwnedNftLimit_TxSeqo);

  ////*************6.invoke contract (test freeMint FT and withdraw)*******************/
  let msgAmount="2.5"; // 2.5 Ton  //send 2.5 ton,should get 2.5 FT and 2 NFT
  let freeMint_TxSeqo = await invokeFreeMintFromTrc404Master(user2_wallet, user2_secretKey,deployMasterContractAddress,msgAmount); 
  console.log("userAddress:", user2_wallet.address.toString(), ",userTxSeqo:", freeMint_TxSeqo);
  await waitNextSeqo(user2_wallet, freeMint_TxSeqo);

  //****withdraw */
  let  withdraw_amount = 1.84;  // 
  let to_address =user2_wallet.address;
  let withdraw_TxSeqo = await invokeWithDrawFromTrc404Master(user1_wallet, user1_secretKey,
    deployMasterContractAddress, withdraw_amount, to_address, "0.01"); 
   console.log("userAddress:", user1_wallet.address.toString(), ",userTxSeqo:", withdraw_TxSeqo);
  await waitNextSeqo(user1_wallet, withdraw_TxSeqo);
