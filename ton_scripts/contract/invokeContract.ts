...
export async function invokeTransferFTFromTrc404Wallet(user_wallet: OpenedContract<WalletContractV4>, secretKey: Buffer, compiledCodes: CompiledCodeList,
    erc404_master_address: Address, receiver_address: Address,
    jetton_amount: number, erc404_nft_collection_address: Address, msg_value_str: string) {
    // ========================================
    ...
    let  wallet_info= await getTrc404WalletAddressAndInit(user_wallet.address, erc404_master_address, compiledCodes.erc404_jetton_wallet_code, erc404_nft_collection_address);
...
export async function invokeTransferNFTFromTrc404NftItem(user_wallet: OpenedContract<WalletContractV4>, secretKey: Buffer, compiledCodes: CompiledCodeList,
    erc404_nft_collection_address: Address, new_owner_address: Address,
    nft_item_index: number, msg_value_str: string) {
    // ========================================
    let msgValueAmount = toNano(msg_value_str);
    let seqno: number = await user_wallet.getSeqno();
    let balance: bigint = await user_wallet.getBalance();
    ...
    await user_wallet.sendTransfer({
        ...
        messages: [
            internal({
                ...
                value: msgValueAmount,
                bounce: true,
                body: buildTransferNftMsg(new_owner_address,user_wallet.address),
            }),
        ],
    });
