...
describe('Test Trc404 NftCollection change_royalty_params,get_royalty_params and change_owned_nft_limit ', () => {
    ...
    it('should not change_owned_nft_limit when sender is not admin', async () => {
        let gasFee = 0.01; //0.15 ton 
        let user1 = await blockchain.treasury("user1");
        let msg = buildChangeOwnedNftLimitMsg(3);
         const changeOwnedNftLimitResult = await Collection.send(user1.getSender(), { value: toNano(gasFee) },msg);  
    ...
    it('should  change_owned_nft_limit when sender is  admin', async () => {
        let gasFee = 0.01; //0.15 ton 
        let msg = buildChangeOwnedNftLimitMsg(3);
         const changeOwnedNftLimitResult = await Collection.send(deployer.getSender(), { value: toNano(gasFee) },msg);  
         expect(changeOwnedNftLimitResult.transactions).toHaveTransaction({
             from: deployer.address,
             to: Collection.address,
             success: true,
          ...
          let collectionInfo =await Collection.getGetFullData();
          expect(collectionInfo.owned_nft_limit).toEqual(3n);
